class video_creator_class {
    ...
    constructor(_engine) {
        ...
        this.preview_halfspeed = document.getElementById('preview_halfspeed')
        ...
        this.preview.addEventListener('touchstart', _ => this.start_previewing({ halfSpeed: false }))
        this.preview.addEventListener('mousedown', _ => this.start_previewing({ halfSpeed: false }))

        this.preview_halfspeed.addEventListener('touchstart', _ => this.start_previewing({ halfSpeed: true }))
        this.preview_halfspeed.addEventListener('mousedown', _ => this.start_previewing({ halfSpeed: true }))

        document.addEventListener('touchend', this.stop_previewing.bind(this))
        document.addEventListener('mouseup', this.stop_previewing.bind(this))
    ...
    start_previewing({ halfSpeed = false } = {}) {
        this.creator_player.currentTime = this.seek_set
        this.creator_player.playbackRate = halfSpeed ? 0.5 : 1
    ...
    init() {
        ...
        this.creator_selection.append(newElement({
            type: 'option',
            attr: {
                value: 'SELECT VIDEO'
            },
